...
import com.android.example.popularmovies.utils.json.JsonUtils.getMovieResultsArray
import com.android.example.popularmovies.utils.json.JsonUtils.getMovieReviews
import com.android.example.popularmovies.utils.json.JsonUtils.getMovieTrailers
import com.android.example.popularmovies.utils.json.JsonUtils.parseMovieJson
import com.android.example.popularmovies.utils.json.JsonUtils.parseMovieReviewsJson
import com.android.example.popularmovies.utils.json.JsonUtils.parseMovieTrailersJson
...
class JsonUtilsTest {
    @Test
    fun parseMovieReviewsJson_OneEntry() {
        //GIVEN
        val testData = JSONArray()
        val exampleAuthor = "Rob"
        val exampleContent = "Bad movie."
        val exampleID = "55d3a610925141201600079c"
        val exampleUrl = "https://themoviedb.org/review/$exampleID"
        val exampleMap = HashMap<String?, String?>()
        exampleMap["author"] = exampleAuthor
        exampleMap["content"] = exampleContent
        exampleMap["id"] = exampleID
        exampleMap["url"] = exampleUrl
        testData.put(JSONObject(exampleMap as Map<*, *>))
        val expectedReview = MovieReview(exampleAuthor, exampleContent, exampleID, exampleUrl)

        //WHEN
        val exampleReviewList: List<MovieReview?>
        exampleReviewList = try {
            parseMovieReviewsJson(testData)
        ...
        //THEN
        assertThat(exampleReviewList[0]).isEqualTo(expectedReview)
    }

    @Test
    fun parseMovieReviewsJson_Empty_ExpectEmpty() {
        //GIVEN
        val testData = JSONArray()
    ...
    @Test
    fun parseMovieReviewsJson_Null_ExpectError() {
        //GIVEN
        val testData: JSONArray? = null
        ...
        exampleReviewList = try {
            ...
            println("Expected NullPointerException caught.")
            return
        }
    }

    @Test
    fun movieReviewParsingFlow_FightClubJSON_ExpectTwoEntries() {
        try {
            //GIVEN
            ...
            //WHEN
            val array = getMovieReviews(testJSON)
            if (array == null) {
                fail("JSON Array is null.")
                return
            }
            val outputReviewList = parseMovieReviewsJson(array)

            //THEN
            assertThat(outputReviewList).hasSize(2)
            assertThat(outputReviewList[0]!!.author).matches("Goddard")
            assertThat(outputReviewList[1]!!.author).startsWith("Brett")
        } catch (e: JSONException) {
            fail(e.message)
        } catch (e: IOException) {
            fail(e.message)
    ...
    @Test
    fun parseMovieTrailersJson_OneEntry() {
        //GIVEN
        val testArray = JSONArray()
        val id = "5c9294240e0a267cd516835f"
        val iso_639 = "en"
        val iso_3166 = "US"
        val key = "BdJKm16Co6M"
        val name = "Fight Club | #TBT Trailer | 20th Century FOX"
        val site = "YouTube"
        val size = 1080
        val type = "Trailer"
        val expectedVideo = MovieVideo(
                id,
                name,
                key,
                size,
        ...
        val equivalentJSON: JSONObject
        val exampleVideoList: List<MovieVideo?>
        try {
            equivalentJSON = JSONObject("{\"id\":\"5c9294240e0a267cd516835f\",\"" +
            ...
            //WHEN
            exampleVideoList = parseMovieTrailersJson(testArray)
        } catch (e: JSONException) {
            fail(e.message)
            return
        } catch (e: IOException) {
        ...
        //THEN
        assertThat(exampleVideoList[0]).isEqualTo(expectedVideo)
        assertThat(exampleVideoList[0]!!.isoIdentifier).matches("en_US")
    }

    @Test
    fun parseMovieVideosJson_Empty_ExpectEmpty() {
        //GIVEN
        val testData = JSONArray()

        //WHEN
        val exampleReviewList: List<MovieVideo?>
        exampleReviewList = try {
            parseMovieTrailersJson(testData)
        } catch (e: JSONException) { //THEN
            fail(e.message)
            return
        } catch (e: IOException) {
    ...
    @Test
    fun movieTrailerParsingFlow_FightClubJSON_ExpectTwoEntries() {
        try {
            //GIVEN
            val testJSON = JSONObject(
            ...
            //WHEN
            val array = getMovieTrailers(testJSON)
            if (array == null) {
                fail("JSON Array is null.")
                return
            }
            val outputVideoList = parseMovieTrailersJson(array)

            //THEN
            assertThat(outputVideoList).hasSize(2)
            assertThat(outputVideoList[0]?.name).contains("Fight Club")
            assertThat(outputVideoList[1]?.name).contains("Fight Club")
        } catch (e: JSONException) {
            fail(e.message)
        } catch (e: IOException) {
            fail(e.message)
